...
* `xi`: tunable parameter $\xi$ of Expected Improvement, to balance exploitation against exploration, increasing `xi` will make the optimized hyper parameters more spread out across the whole range
...
```
The sum slightly exceeds 1, which is probably the reason why our Sharpe's Ratio is negative. More work can be done to improve sampling for next $x$ as well as finding the optimum value for parameters $l$ and $\sigma_f$. 
...
```
The solution has a Sharpe Ratio of `r round(bayes_finance_norm$value, 4)`. We achieve a higher performance than both `rBayesianOptimization` and Particle Swarm Optimization, and in just one iteration!
